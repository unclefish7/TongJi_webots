<template>
    <div class="voice-recorder">
        <!-- 录音按钮 -->
        <el-button :type="isRecording ? 'danger' : 'primary'" :loading="isProcessing" @click="toggleRecording"
            :disabled="!isSupported" size="large" class="record-button">
            <el-icon>
        ...
        <div v-if="recognitionResult" class="recognition-result">
            <el-card class="result-card">
                ...
                <div class="result-content">
                    ...
                    <div v-if="recognitionResult.analysis_method" class="analysis-method">
                        <strong>分析方式:</strong>
                        <el-tag :type="getAnalysisMethodType(recognitionResult.analysis_method)" size="small">
                            {{ getAnalysisMethodText(recognitionResult.analysis_method) }}
                        </el-tag>
                    </div>
                    ...
                    <div v-if="recognitionResult.matched_keywords.length > 0" class="matched-keywords">
                        <strong>匹配关键词:</strong>
                        <el-tag v-for="keyword in recognitionResult.matched_keywords" :key="keyword" size="small"
                            class="keyword-tag">
                            {{ keyword }}
                        </el-tag>
                    </div>
                ...
                <div class="result-actions">
                    <el-button v-if="recognitionResult.location" type="primary" @click="selectLocation">
                        选择此地点
                    </el-button>
        ...
        <!-- 帮助信息 -->
        <div class="help-info">
            <el-alert title="语音指令示例" type="info" :closable="false" class="help-alert">
                <template #default>
...
const getAnalysisMethodType = (method: string): string => {
    switch (method) {
        case 'Offline-LLM':
            return 'success'
        case 'Online-LLM':
...
const getAnalysisMethodText = (method: string): string => {
    switch (method) {
        case 'Offline-LLM':
            return '离线AI分析'
        case 'Online-LLM':
