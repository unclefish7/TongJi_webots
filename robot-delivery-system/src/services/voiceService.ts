...
export interface VoiceRecognitionResult {
    ...
    analysis_method?: 'Offline-LLM' | 'Online-LLM' | 'Keywords'  // 分析方法
    filename?: string
    file_size?: number
    content_type?: string
}
...
export class VoiceRecognitionService {
    ...
    async recognizeAudio(audioFile: File): Promise<VoiceRecognitionResult> {
        try {
            ...
            const response = await axios.post(`${API_BASE_URL}/api/voice/recognize`, formData, {
                headers: {
    ...
    async getAvailableLocations(): Promise<{ locations: Record<string, string[]>, location_names: string[] }> {
        try {
            const response = await axios.get(`${API_BASE_URL}/api/voice/locations`)
            return response.data
        } catch (error) {
    ...
    async testTextExtraction(text: string): Promise<any> {
        try {
            const response = await axios.post(`${API_BASE_URL}/api/voice/test-recognition?text=${encodeURIComponent(text)}`)
            return response.data
        } catch (error) {
